...
window.addEventListener('load', async function() {
    ...
    const isUseAllowed = await chrome.runtime.sendMessage({type: 'is_use_allowed'});

    console.log(isUseAllowed);
    ...
    addAccountButton.addEventListener('click', function() {
        chrome.runtime.sendMessage({type: 'add_account'});
        window.close();
    })

    const accountsData = await chrome.runtime.sendMessage({type: 'get_accounts_data'});
    ...
    for (const userId in accountsData) {
        ...
        frame.addEventListener('click', function () {
            chrome.runtime.sendMessage({type: 'switch_account', userId: userId});
            window.close();
        })
    ...
    removeAccountButton.addEventListener('click', function() {
        chrome.runtime.sendMessage({type: 'remove_account'});
        window.close();
    })
})